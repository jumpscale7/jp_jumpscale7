...
class Actions(ActionsBase):
    ...
    def start(self,hrd,**args):
        ...
        do.execute("killall nginx")
        j.system.process.killProcessByPort(80)
        cmd="cd /opt/code/git/binary/lemp/bin;./nginx -c /opt/lemp/cfg/nginx/nginx.conf"
        do.execute(cmd)
        
        # import JumpScale.baselib.screen
        # j.system.platform.screen.createSession("servers2",["nginx"])
        # j.system.platform.screen.executeInScreen(sessionname="servers", screenname="nginx", cmd=cmd, wait=0, cwd=None, env=None, user='root', tmuxuser=None)

        #now wait till we can access the port
        res=j.system.net.waitConnectionTest("localhost",80,2)
        if res==False:
            j.events.inputerror_critical("nginx did not become active, check in byobu","jpackage.install.nginx.startup")
